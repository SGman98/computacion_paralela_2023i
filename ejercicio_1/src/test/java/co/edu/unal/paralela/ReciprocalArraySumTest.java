...
public class ReciprocalArraySumTest extends TestCase {
  // Número de veces que se debe repetir cada prueba para dar consistencia de los resultados de en
  // el tiempo.
  private static final int REPEATS = 60;
  ...
  /**
   * Una implementación de referencia de seqArraysum, en caso de que alguno del archvo del código
   * fuente principal sea modificado accidentalmente.
   *
  ...
  /**
   * Una función 'helper' para hacer las pruebas de la implementación de dos y la implementación de
   * muchas tareas en paralelo.
   *
   * @param N Tamaño del arreglo utilizado para las pruebas
   * @param useManyTaskVersion Switch entre el código la versión de dos tareas en paralelo y la
   *     versión de muchas tareas en paralelo
   * @param ntasks Número de tareas a utilizar
   * @return La mejora en la rapidez (speedup) alcanzada, no todas las pruebas utilizan esta
   *     información
   */
  private double parTestHelper(final int N, final boolean useManyTaskVersion, final int ntasks) {
    ...
    final String errMsg =
        String.format(
            "No concuerda el resultado para N = %d, valor esperado = %f, valor calculado = %f,"
                + " error absoluto = %f",
            N, correct, sum, err);
  ...
  /**
   * Prueba que la implementación de dos tareas en paralelo calcula correctamente los resultados
   * para arreglos con un millón de elementos.
   */
  public void testParSimpleTwoMillion() {
    ...
    final String errMsg =
        String.format(
            "Se esperaba que la implementación de dos tareas en paralelo pudiera ejecutarse  %fx"
                + " veces más rápido, pero solo alcanzo a mejorar la rapidez (speedup) %fx veces",
  ...
  /**
   * Prueba que la implementación de dos tareas en paralelo calcula correctamente los resultados
   * para arreglos con cientos de millones de elementos..
   */
  ...
  /**
   * Prueba que la implementación de muchas tareas en paralelo calcula correctamente los resultados
   * para arreglos con un millónde elementos.
   */
  public void testParManyTaskTwoMillion() {
    ...
    final String errMsg =
        String.format(
            "Se esperaba que la implmentación de muchas tareas en paralelo pudiera ejecutarse %fx"
                + " veces más rápido, pero solo alcanzo a mejorar la rapidez (speedup) %fx veces",
            minimalExpectedSpeedup, speedup);
    assertTrue(errMsg, speedup >= minimalExpectedSpeedup);
  }
